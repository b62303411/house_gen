...
class CollisionBox:
    ...
    def get_direction(self):
        if self._cached_direction is None:
            ...
            self._cached_direction = direction.normalize()
        return self._cached_direction.copy()
    ...
    @staticmethod
    def snap_angle_to_raw(angle_deg):
        """Snap angle to the nearest raw direction angle (0, 45, ..., 315)."""
        return min(Constants.raw_directions.keys(), key=lambda d: abs((angle_deg - d + 180) % 360 - 180))
    ...
    def create_from_line(line, width):
        ...
        # Snap to closest raw direction
        snapped_angle_deg = CollisionBox.snap_angle_to_raw(angle_deg)
    ...
    def calculate_overlap(self, other):
        polygon_self = self.get_polygon()
        polygon_other = other.get_polygon()
    ...
    def iterate_covered_pixels(self):
        ...
        for x in range(min_x, max_x + 1):
            for y in range(min_y, max_y + 1):
                center = Point(x, y)  # Test pixel center
                if wall_polygon.buffer(epsilon).contains(center):
